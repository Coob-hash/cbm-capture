...
| POST | `/v1/auth/login/google` | — | Google ID token → one-hour session. An address a password account holds links to that account, and Google having proved the mailbox, the password and every session opened with it end: whoever registered the address first cannot log in with it again, nor read what the owner writes from then on. The account signs in with Google from then on. A disabled account is not linked |
